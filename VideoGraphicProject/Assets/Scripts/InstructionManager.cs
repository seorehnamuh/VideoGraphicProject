...
public class InstructionManager : MonoBehaviour
{
    ...
    // Metodo per gestire la collisione con il Player, quando il giocatore entra in contatto con il pannello
    private void OnCollisionEnter(Collision collision)
    {
        if (collision.gameObject.CompareTag("Player"))
        {
            Debug.Log("Il giocatore ha colpito il pannello!");
            // Se il pannello è nel layer giusto, incrementa il contatore
            if (gameObject.layer == LayerMask.NameToLayer("Pannello"))
            {
                Debug.Log("Il pannello è nel layer giusto!");
                PanelChecked();  // Chiama la funzione per incrementare il conteggio dei pannelli controllati
            }
            else
            {
